							...
							<div class="doctor-widget">
								<div class="doc-info-left">
									...
									<div class="doc-info-cont">
										<h4 class="doc-name">{{ $lawyer->user->name }}</h4>
										<p class="doc-speciality">{{ $lawyer->education }}</p>
										<p class="doc-department">
                                            @if(isset($lawyer->lawyerType->image))
                                                <img src="{{ asset('storage/'.$lawyer->lawyerType->image) }}" class="img-fluid" alt="Speciality">
                                            @endif
                                            {{ $lawyer->lawyerType->name }}</p>
                                        <div class="rating">
														...
														<li>
															...
															<div class="experience-content">
																<div class="timeline-content">
																	<a href="#/" class="name">{{ $lawyer->education }}</a>
																	<span class="time">{{ $lawyer->passing_year }}</span>
																</div>
															</div>
														</li>
													</ul>
								...
								<div role="tabpanel" id="doc_reviews" class="tab-pane fade">
									...
									<div class="widget review-listing">
										<ul class="comments-list">
											...
											<!-- Comment List -->
                                            @foreach($lawyer->user->lawyerReviews as $review)
                                                <li>
                                                    <div class="comment">
                                                        @if(isset($review->user->image))
                                                        <img class="avatar avatar-sm rounded-circle" alt="User Image" src="{{ asset('storage/'.$review->user->image) }}">
                                                        @endif
										...
										<div class="all-feedback text-center">
											<a href="#" class="btn btn-primary btn-sm">
												Show all feedback <strong>{{ count($lawyer->user->lawyerReviews) }}</strong>
											</a>
										</div>
										<!-- /Show All -->
                                    ...
                                    @can('book lawyer')
                                        @if(Auth::check() || Auth::user()->hasRole(Config::get('constants.roles.User')))
                                            <div class="write-review">
                                                <h4>Write a review for <strong>{{ $lawyer->user->name }}</strong></h4>

                                                <!-- Write Review Form -->
                                                <form id="review_form">
                                                    @csrf
                                                    <div class="form-group">
                                                        <input type="hidden" name="lawyer_id" value="{{ $lawyer->user->id }}">
                                                        <label>Review</label>
